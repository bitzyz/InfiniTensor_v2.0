...
TEST_F(TensorBasicTest, ConstructWithStride) {
  ...
  // 从常量步长构造
  auto tensor1 =
      make_ref<TensorObj>(concreteDims, strideDims, DataType(INFINI_DTYPE_U32));
  ...
  auto tensor2 =
      make_ref<TensorObj>(concreteDims, strideExpr, DataType(INFINI_DTYPE_U32));
  EXPECT_EQ(tensor2->getStride()->toString(), "[12, 4, 1]");
}

// 测试形状获取和设置
TEST_F(TensorBasicTest, ShapeGetSet) {
  auto tensor = make_ref<TensorObj>(Shape{2, 3, 4}, DataType(INFINI_DTYPE_F32));
